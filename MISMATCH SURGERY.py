...
Cpm = Cpm[Rmin:Rmax, :]
Ctm = Ctm[Tmin:Tmax, :]
normalisedCpm = normalisedCpm[Rmin:Rmax, :]
normalisedCtm = normalisedCtm[Tmin:Tmax, :]
...
plt.subplot(2, 2, 2)
for m in range(M):
    plt.plot(range(Tmin + 1, Tmax + 1), Ctm[0:nT, m])
...
plt.xlabel('Postsynaptic Cell Number')
plt.xticks(range(0, nT + 1, 10), range(Tmin, Tmax + 1, 10))
plt.yticks(range(0, nR + 1, 10), range(Rmin, Rmax + 1, 10))
