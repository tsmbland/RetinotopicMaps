...
deltaWpt = np.zeros([nT, nR])
deltaWsum = np.zeros([nR])
Qpm = np.resize(Qpm, (nR, M))
Qtm = np.resize(Qtm, (nT, M))
Cpm = np.resize(Cpm, (nR, M))
Ctm = np.resize(Ctm, (nT, M))
normalisedCpm = np.resize(normalisedCpm, (nR, M))
normalisedCtm = np.resize(normalisedCtm, (nT, M))
Spt = np.zeros([nT, nR])
meanSp = np.zeros([nR])
